...
def answer_cleansing(args, pred, must_choice=False):
    ...
    if args.method in ("few_shot", "few_shot_cot", "auto_cot"):
        preds = pred.split(args.direct_answer_trigger_for_fewshot)  #分割preds
    ...
    if pred != "":
        if pred[-1] == ".":
            pred = pred[:-1]    
    print("pred_after : " + ', '.join(map(str, pred)))
    # print("pred_after : " + pred)
  #pred是list型態，要用''.join迭代後變成字串型態
    return pred

def create_demo_text(args, cot_flag):
    x, z, y = [], [], []
    
    with open(args.demo_path, encoding="utf-8") as f: #multiarith_auto字典
        json_data = json.load(f)
        json_data = json_data["demo"]   
        for line in json_data:
            x.append(line["question"])   #提取questions
            z.append(line["rationale"])  #提取rationale
            y.append(line["pred_ans"])   #提取預測答案。  不需要正確答案,因為是auto
    ...
    return demo_text
# create_demo_text(args,cot_flag=True)
def answer_cleansing_zero_shot(args, pred, must_choice=False):
    ...
    #     pred = re.sub("\"|\'|\n|\.|\s|\:|\,", " ", pred)
    #     pred = pred.split(" ")
    #     pred = [i for i in pred if i in ("yes", "no")]  #只保留yes跟no
    # elif args.dataset == "last_letters":
    #     pred = re.sub("\"|\'|\n|\.|\s", "", pred)   #去除各種符號
    #     pred = [pred]
    else:
