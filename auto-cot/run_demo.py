...
def parse_arguments():
    ...
    parser.add_argument(
        "--task", type=str, default="multiarith",  #預設任務
...
def main():
    args = parse_arguments()    # 我要執行哪個任務？限制？
    ...
    if task == "last_letters":
        max_ra_len = 7
    if task == "aqua" or task == "last_letters":  #依照任務分成不同群數
        num_clusters = 4
    ...
    with open(pred_file, "r", encoding="utf-8") as fp: #pred_file --> multiarith_zero_shot_cot.log
        answer_seg = ""
        for line in fp:
            if "Q: " in line:
                c_question = line.strip() #取出Questions
            if "A: " in line:
                answer_seg = line   #取出Answers (Let's think step by step)
            elif "Therefore" in line and "the answer" in line:
                c_rationale = answer_seg    # Let's think step by step
            elif answer_seg != "":
                answer_seg += line

            if "pred_mode" in line:
                c_pred_ans = line.split(":")[1].strip() # 取預測答案
            if "GT :" in line:
                c_gold_ans = line.split(":")[1].strip() # 取正確答案
    ...
    corpus_embeddings = encoder.encode(corpus)    #corpus拿來裝c_questions -->經過encoder後定義為corpus_embedding

    # Perform k-means clustering
    clustering_model = KMeans(n_clusters=num_clusters, random_state=args.random_seed)
    clustering_model.fit(corpus_embeddings)
    cluster_assignment = clustering_model.labels_   #最佳labels(best labels)(inertia族內族內誤差平方和-->inertia越小越密集)

    clustered_sentences = [[] for i in range(num_clusters)]

    dist = clustering_model.transform(corpus_embeddings) #計算嵌入後的向量和K-means中心的距離 before聚類
    clustered_dists = [[] for i in range(num_clusters)] 
    clustered_idx = [[] for i in range(num_clusters)]
    for sentence_id, cluster_id in enumerate(cluster_assignment):       #挑出每個cluster的最佳label的....
        clustered_sentences[cluster_id].append(corpus[sentence_id])    #的內容(question)
        clustered_dists[cluster_id].append(dist[sentence_id][cluster_id]) #和中心的距離  after聚類
        clustered_idx[cluster_id].append(sentence_id) #的id

    demos = []

    for i in range(len(clustered_dists)):
        print("Cluster ", i+1)
        tmp = list(map(list, zip(range(len(clustered_dists[i])), clustered_dists[i]))) #tmp -->[,0~7]
        top_min_dist = sorted(tmp, key=lambda x: x[1], reverse=False) #升序排列tmp第二個值
            ...
            if len(question[clustered_idx[i][min_idx]].strip().split()) <= 60 \
                ...
                if args.task in ["gsm8k", "multiarith", "singleeq", "addsub", "svamp"]:
                    if not (c_pred_ans.strip() in c_rationale.split(".")[-2] or c_pred_ans.strip() in c_rationale.split()[-10:]):
                        continue
                c_question = question[clustered_idx[i][min_idx]]  #question[分成幾個類][top_min_dist]
                ...
                print(c_question)   # Q:.... \n A:
                print(c_rationale)  # Let's think step by step. ......
                print(c_pred_ans)   # 預測答案
                print(c_gold_ans)   # 正確答案
                print("")
                break

    demos = {"demo": demos}

    with open(args.demo_save_dir, 'w', encoding="utf-8") as write_f: #demo/multiarith
